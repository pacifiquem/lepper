...
export const init = (
  ...
  if (folders != undefined && directory != undefined) {
    ...
    folderNames = folderNames.filter(
      (item, index) => folderNames.indexOf(item) === index
    );
...
export const generate = (files: string[], directory: string) => {
  if (files != undefined && directory != undefined) {
    ...
    fileNames = fileNames.filter(
      (item, index) => fileNames.indexOf(item) === index
    );
    fileNames.map((name) => {
...
export const descriptionManager = (argument: {
  ...
  if (argument.hasOwnProperty("file")) {
    ...
    if (filesInDirectory.includes(fileName)) {
      fs.appendFile(
        `${programPath}/programm-data/files.txt`,
  ...
  if (argument.hasOwnProperty("folder")) {
    ...
    if (foldersInDirectory.includes(folderName)) {
      fs.appendFile(
        `${programPath}/programm-data/folders.txt`,
  ...
  if (argument.hasOwnProperty("project")) {
    ...
    fs.appendFile(
      `${programPath}/programm-data/projects.txt`,
